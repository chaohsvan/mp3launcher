...
- 可选“模拟屏幕操作”：
  - 在模拟 LCD 屏幕左滑下一首，右滑上一首，并带短促滑动反馈动画。
  - 长按专辑封面会打开当前正在播放媒体所属的 App，例如 Musicolet、小宇宙等。
...
在设置中开启“在其他应用中也用音量键切歌”后，`Track + play` / `<>` 拨杆模式会扩展到其他前台应用：

- 其他应用前台时，音量上短按映射为上一首，音量下短按映射为下一首。
- 长按任意音量键会触发播放/暂停。
- 该功能依赖无障碍服务的按键过滤权限，且不会读取屏幕内容。
...
- 通知访问权限入口。
- 无障碍设置入口，并说明用途：用于锁屏实体音量键切歌，以及可选的其他应用前台音量键切歌；不读取屏幕内容。
- 默认桌面设置入口。
- 未播放时默认音乐 App 选择。
- 实体音量键模式选择。
- 在其他应用中也用音量键切歌开关。
- 极简模式开关。
- 开机动画开关。
- 模拟屏幕操作开关，控制模拟屏幕横滑切歌和长按封面打开当前媒体 App。
- 主题皮肤切换。
- 置顶应用管理。
- 隐藏应用管理。
- 选择要隐藏的应用。
...
用于在锁屏亮屏状态捕获实体音量键，让 MP3 Launcher 可以把音量键映射为上一首/下一首/播放暂停。开启“在其他应用中也用音量键切歌”后，也用于在其他应用前台时处理实体音量键。服务声明 `canRetrieveWindowContent=false`，不会读取屏幕内容。
...
- 无障碍服务只用于实体音量键过滤，不提供屏幕读取、自动点击或界面分析能力。
- 其他应用前台音量键切歌依赖 Android 无障碍按键过滤；若系统或厂商限制该能力，行为可能与主界面不同。
...
- 模拟屏幕横滑和封面长按入口由 `MainActivity.ui.kt` 中的模拟屏幕触摸逻辑处理。
